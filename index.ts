...
async function plotTestingGraph(){
  let tdata;
  if(!ls.getItem('tdata')){
...
function generateWorldMap(){
  let series = data.getCountrySeries();
  var dataset = {};
  ...
  if(elem.children.length==0){
    ...
    window.wmap = wmap;
    let {tcn,tdn,ncn,ndn} = data.getWorldHighlights();
...
function generateIndia(){
  let series = data.getIndiaSeries();
  var dataset = {};
  var onlyValues = series.map(function(obj){ return obj[1]; });
  var minValue = Math.min.apply(null, onlyValues),
      maxValue = Math.max.apply(null, onlyValues);
...
indbtn.addEventListener("click",(ev) => {
  let elem = document.getElementById("india-info");
  routeHelper(elem);
  listHelper(ev);
  if(elem.classList.contains("d-none")){
    elem.classList.remove("d-none");
