...
const indDistrictWise = "https://api.covid19india.org/state_district_wise.json";
...
export function getUKHighlights(){
	let stateWise = JSON.parse(ls.getItem("indStateWise"));
	let ukObj = stateWise.filter(o => o.state=="Uttarakhand");
	let tcn = ukObj[0]["confirmed"];
	let tdn = ukObj[0]["deaths"];
	let ncn = ukObj[0]["delta"]["confirmed"];
	let ndn = ukObj[0]["delta"]["deaths"];
	let rec = ukObj[0]["recovered"];
	let act = ukObj[0]["active"];
	// let ttest = tested[tested.length-1]["totalindividualstested"];
	// let utest = tested[tested.length-1]["updatetimestamp"];
	return {tcn,tdn,ncn,ndn,rec,act};
}
...
// UK => Uttarakhand
export function getUKData(){
	return fetch(indDistrictWise)
				.then(resp => resp.json())
				.then(json => {
					let ukData = json.Uttarakhand;
					ls.setItem("ukDistrictWise",JSON.stringify(ukData));
				});
}
